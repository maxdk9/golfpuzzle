...
public class MyFileBrowser : MonoBehaviour
{
	...
	public void LoadTexture(Action<Texture2D> callback)
	{
		...
		this.StartCoroutine(ShowLoadDialogCoroutine(callback));
	...
	{
		this.StartCoroutine(ShowSaveDialogRoutine(texture));
	}



	IEnumerator ShowSaveDialogRoutine(Texture2D texture)
	{
		LevelEditor levelEditor = FindObjectOfType<LevelEditor>();
		levelEditor.EditorBoard.gameObject.SetActive(false);
		
		yield return FileBrowser.WaitForSaveDialog(false, false, "Save file", "Save");
		...
		levelEditor.EditorBoard.gameObject.SetActive(true);
	}
	
	
	IEnumerator ShowLoadDialogCoroutine(Action<Texture2D> callback)
	{
